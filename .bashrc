#  ---------------------------------------------------------------------------
#
#  Description:  This file holds all BASH configurations and aliases
#
#  Sections:
...
#   Set Paths: This is only for paths not in /etc/paths
#   ------------------------------------------------------------
    export PATH="$PATH:/usr/local/mysql/bin:/usr/local/MacGPG2/bin:/Applications/DevDesktop/drush"
...
#   -----------------------------
    ...
    alias fix_stty='stty sane'                # fix_stty: Restore terminal settings
    alias cic='set completion-ignore-case On' # cic: Make tab-completion case-insensitive
    mcd () { mkdir -p "$1" && cd "$1"; }      # mcd: Makes new Dir and jumps inside
    ql () { qlmanage -p "$*" >& /dev/null; }  # ql: Opens any file in Preview
...
#   -------------------------------------------------------------------
    extract(){
	  ...
	  *.Z)		    uncompress $1	;;
	  *.7z)		    7z x $1		;;
	  *)	echo "'$1' cannot be extracted vi extract()" ;;
	  esac
       else
...
#   ---------------------------
    ...
    ffs () { /usr/bin/find . -name "$@"'*' ; }  # Find file with a given string
    ffe () { /usr/bin/find . -name '*'"$@" ; }  # Find file ends with a given string
...
#   ---------------------------
    ...
    alias myip='curl ipinfo.io/ip' # Public facing IP Address
    ...
    alias showBlocked='sudo ipfw list' # All ipfw rules include/ blocked IPs
